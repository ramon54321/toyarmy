...
public class GPUProfiler {
    ...
    private static ObjectPool<GPUTaskProfile> taskPool;
    ...
    static{
        taskPool = new ObjectPool<GPUTaskProfile>(4, 50, 5) {
            @Override
            protected GPUTaskProfile createObject() {
    ...
    public static void startFrame(){
        ...
        if(PROFILING_ENABLED){
            currentTask = taskPool.borrowObject().init(null, "Frame " + (++frameCounter), getQuery());
        }
    }

    public static void start(String name){
        if(PROFILING_ENABLED && currentTask != null){
            currentTask = taskPool.borrowObject().init(currentTask, name, getQuery());
        }
    }
