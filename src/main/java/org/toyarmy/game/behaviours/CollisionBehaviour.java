...
public class CollisionBehaviour extends Behaviour {
    ...
    public ArrayList<LineSegment> collisionSegments = new ArrayList<>();
    ...
    public ArrayList<LineSegment> getCollisionSegmentsWithRotation(){
        float rotation = (float) Math.toRadians(transformComponent.getRotation());
        Matrix3f rotationMatrix = new Matrix3f().rotateZ(-rotation);

        ArrayList<LineSegment> collisionSegmentsRotated = new ArrayList<>();

        for(LineSegment segment : collisionSegments) {
            ...
            collisionSegmentsRotated.add(rotatedSegment);
        }

        return collisionSegmentsRotated;
    }
